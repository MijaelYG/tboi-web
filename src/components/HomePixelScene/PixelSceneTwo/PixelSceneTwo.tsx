...
const PixelSceneTwo = ({ScrollYProgress,ScrollStartEnd}: PropsPixelScene) => {
    ...
    const move2 = useTransform(ScrollYProgress,[start,end],[0,-80])
    const animation2smooth = useSpring(animation2,{stiffness:200, damping:40})
    ...
    const animation3smooth = useSpring(animation3,{stiffness:200, damping:40})
    ...
    const move3value = useTransform(move3smooth, (value) => `${value}%`)

    const animation4 = useTransform(ScrollYProgress, [start,end],[3,30]);
    const move4 = useTransform(ScrollYProgress,[start,end],[0,100])
    const animation4smooth = useSpring(animation4,{stiffness:200, damping:40})
    const move4smooth = useSpring(move4,{stiffness:300, damping:40})
    const value4 = useTransform(animation4smooth, (value) => `calc(-${6.5}vw *${Math.floor(value) % 10})`);
    const move4value = useTransform(move4smooth, (value) => `${value}%`)
  ...
  return (
    ...
    <motion.div className={style.pixel_isaac} style={{backgroundPositionX:value3, x:move3value}}>
    </motion.div>
    <motion.div className={style.pixel_monster3} style={{backgroundPositionX:value4, x:move4value,scaleX:-1}}>
    </motion.div>
    </>
  )
}
